...
try:
	...
	r = requests.post(Host + URL_UPDATE, data = payload_update, auth=HTTPBasicAuth('django', 'django'))
	#log it
	...
	for index, row in mudata.iterrows() :
	    ...
	    if (row['Topic'] != last_category) :
	    	...
	    	r = requests.post(Host + URL_CATEGORY, data = payload_category, auth=HTTPBasicAuth('django', 'django'))
	    ...
	    if (row['Ticker'] != last_symbol) :
	    	...
	    	r = requests.post(Host + URL_SYMBOL, data = payload_symbol, auth=HTTPBasicAuth('django', 'django'))
	    ...
	    r = requests.post(Host + URL_VALUES, data = payload_value, auth=HTTPBasicAuth('django', 'django'))
